            ...
            init();
				...
				<script type='text/javascript'>
					...
					<xsl:for-each select="/BugCollection/FindBugsSummary/PackageStats/ClassStats">
						...
						[ "<xsl:value-of select="@class"/>", "<xsl:value-of select="../@package"/>", "<xsl:value-of select="@interface"/>", "<xsl:value-of select="@bugs"/>", "<xsl:value-of
							select="@priority_1"/>", "<xsl:value-of select="@priority_2"/>", "<xsl:value-of select="@priority_3"/>", "<xsl:value-of select="@priority_4"/>" ],
					</xsl:for-each>
					[ "", "", "", "", "", "", "", "" ]
					);
					...
					<xsl:for-each select="/BugCollection/FindBugsSummary/PackageStats">
						...
						[ "<xsl:value-of select="@package"/>", "<xsl:value-of select="@total_bugs"/>", "<xsl:value-of select="@priority_1"/>", "<xsl:value-of select="@priority_2"/>", "<xsl:value-of
				...
				<div id='bug-collection' style='display:none;'>
					...
					<xsl:for-each select="/BugCollection/BugInstance[not(@last)]">
						<div style="display:none;" class='bug'>
							<xsl:attribute name="id">b-uid-<xsl:value-of select="@instanceHash"/>-<xsl:value-of select="@instanceOccurrenceNum"/>
							</xsl:attribute>
