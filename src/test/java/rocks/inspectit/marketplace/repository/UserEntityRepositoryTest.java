...
public class UserEntityRepositoryTest extends AbstractTransactionalJUnit4SpringContextTests {
	...
	public void findAllWithProducts() throws Exception {
		...
		assertThat(list.get(0).getProductEntityList().size(), CoreMatchers.is(6));
		assertThat(list.get(1).getProductEntityList().size(), CoreMatchers.is(3));
	...
	public void findAllWithrRatings() throws Exception {
		List<UserEntity> list = (List<UserEntity>) this.repository.findAll();
		assertThat(list.get(0).getRatingEntityList().size(), CoreMatchers.is(7));
		assertThat(list.get(1).getRatingEntityList().size(), CoreMatchers.is(7));
